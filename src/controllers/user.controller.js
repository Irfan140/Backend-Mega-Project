...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  //TODO: delete old image - assignment (we have to make a utility function for this)
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  // getting user details from req.params
  ...
  const channel = await User.aggregate([
    ...
    // user subscriptions to other channels
    {
      $lookup: {
        from: "subscriptions",
        localField: "_id",
    ...
    // selected will be passed down to next stage using 1
    {
      $project: {
        fullName: 1,
        username: 1,
...
// user watch history details
const getWatchHistory = asyncHandler(async (req, res) => {
  const user = await User.aggregate([
    // aggregation code are directly passed so we need to make mongodb id (we cannot directly pass req.user._id )
    {
      $match: {
